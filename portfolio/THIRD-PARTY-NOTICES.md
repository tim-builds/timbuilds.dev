...
## Windows Vista, Windows 10 and Windows 11 appearance

The Vista Aero, Windows 10 and Windows 11 browser shells are independent CSS/JavaScript reimplementations. They are not Windows installations and are not affiliated with Microsoft. The Windows 11 option deliberately follows the recognizable launch-era centered Start / pinned-app layout; it does not claim to reproduce every feature of a current Windows release.

The six new wallpaper files and their derivations are recorded in `wallpapers/modern-provenance.json`, including source/member, original dimensions and SHA-256 checksums. Vista Aurora and the warm glow picture were extracted unchanged from `Windows Vista-Server 2008.zip` in https://archive.org/details/windows-wallpapers . The two Windows 10 images were retrieved from that collection's `Windows 10-Server 2016-Server 2019-Server 2022.zip` members `img0 (Dark).jpg` and `img0 (Light).jpg`. These Vista / Windows 10 archive copies retain their original 800-pixel width; they are not advertised as high-resolution remasters.

Windows 11 Bloom light and dark were derived from the Windows-bundled `Windows/Web/Wallpaper/Windows/img0.jpg` and `img19.jpg`, respectively. They were proportionally resized to 2560 pixels wide and encoded at JPEG quality 90. Each of the six images has a small 300-pixel picker thumbnail. Microsoft and the respective creators retain copyright in these images. They are not public domain and are not covered by the site's software or pinball licences. No Windows binaries, extracted skin resources, icon packs or font files are bundled.

`modern-icons.svg` contains hand-authored vector re-creations for this browser desktop, including Windows-style Start glyphs. The existing independent-site trademark notice continues to apply. All wallpaper and icon requests stay on the website's own origin.

Design references: Vista composition/glass, https://learn.microsoft.com/en-us/windows/win32/w8cookbook/desktop-window-manager-is-always-on ; Windows 11 introduction, https://blogs.windows.com/windowsexperience/2021/06/24/introducing-windows-11/ ; Bloom design background, https://blogs.windows.com/devices/2022/11/10/surface-special-edition-merges-new-and-classic-designs-with-windows-11-bloom-and-liberty/ . These are reference links only, not runtime dependencies.
