...
(() => {
  ...
  function updateClock(){const now=new Date();$('#clock').textContent=window.TimClock.format(now);$('#clock').dateTime=now.toISOString();$('#clock').title=now.toLocaleDateString([],{weekday:'long',year:'numeric',month:'long',day:'numeric'});$('#year').textContent=String(now.getFullYear());}
