...
(() => {
  ...
  const isWebsiteLink = url => /^(?:https:\/\/|\/(?!\/))/i.test(url);
  const external = (url, text) => `<a class="bevel-button primary-button" href="${safe(url)}"${isWebsiteLink(url)?' data-project-website target="_blank" rel="noopener noreferrer"':''}>${safe(text)} <span aria-hidden="true">↗</span>${isWebsiteLink(url)?'<span class="sr-only"> (opens in a new browser tab)</span>':''}</a>`;
  ...
  document.addEventListener('click',async event => {
    ...
    else if(el.hasAttribute('data-preview-project')) window.TimBrowser?.open(el.dataset.previewProject);
