...
(() => {
  ...
  const compact=()=>innerWidth<=820;let left=12,cellW=104,cellH=96,mobileMode=compact(),desktopPositions={},mobilePositions={},selectMode=false,touch=null;
  ...
  dock.addEventListener('pointerdown',event=>{
    pointerType=event.pointerType||'mouse';if(event.target.closest('.touch-desktop-tools'))return;if(pointerType==='touch'||event.button!==0||document.querySelector('dialog[open]'))return;
  ...
  document.addEventListener('click',event=>{
    ...
    if(selectMode&&pointerType==='touch'&&event.detail>0){event.preventDefault();event.stopImmediatePropagation();const id=el.dataset.shortcut;if(selected.has(id))selected.delete(id);else selected.add(id);paintSelection();return;}
  ...
  const tools=document.createElement('div');tools.className='touch-desktop-tools';tools.innerHTML='<button id="desktop-select-toggle" aria-pressed="false">Select icons</button><button id="desktop-arrange-touch">Arrange</button>';dock.append(tools);
  function setSelectionMode(value){selectMode=!!value;dock.classList.toggle('select-mode',selectMode);tools.querySelector('#desktop-select-toggle').textContent=selectMode?'Done':'Select icons';tools.querySelector('#desktop-select-toggle').setAttribute('aria-pressed',String(selectMode));if(!selectMode)selectOnly(null);}
  tools.addEventListener('click',e=>{if(e.target.id==='desktop-select-toggle')setSelectionMode(!selectMode);if(e.target.id==='desktop-arrange-touch')arrange();});
  function cancelTouch(){if(touch)clearTimeout(touch.timer);touch=null;}
  function beginTouch(){if(!touch)return;const t=touch;setSelectionMode(true);window.TimWindows.deactivate();const id=t.el?.dataset.shortcut;if(id&&!selected.has(id))selected=new Set([id]);paintSelection();const initial=new Set(selected),start=Object.fromEntries([...selected].map(id=>[id,{...positions[id]}]));if(!t.el)selectOnly(null);gesture={id:t.id,el:t.el,modifier:false,initial,start,x:t.x,y:t.y,moved:false};t.active=true;box.hidden=!!t.el;Object.assign(box.style,{left:t.x+'px',top:t.y+'px',width:'1px',height:'1px'});}
  dock.addEventListener('touchstart',e=>{pointerType='touch';if(e.target.closest('.touch-desktop-tools'))return;if(e.touches.length!==1){cancelTouch();finish({type:'cancel'});return;}const t=e.touches[0];touch={id:t.identifier+10000,el:e.target.closest('.desktop-shortcut'),x:t.clientX,y:t.clientY,active:false};touch.timer=setTimeout(beginTouch,selectMode?0:350);},{passive:true});
  ...
  dock.addEventListener('touchend',e=>{if(!touch)return;if(touch.active){e.preventDefault();finish({type:'pointerup',pointerId:touch.id});suppressUntil=performance.now()+600;}cancelTouch();},{passive:false});
  dock.addEventListener('touchcancel',()=>{cancelTouch();finish({type:'cancel'});},{passive:true});
  dock.addEventListener('contextmenu',e=>{if(pointerType==='touch'&&(touch?.active||selectMode))e.preventDefault();});
  window.addEventListener('timbuilds-session-reset',()=>{cancelTouch();finish({type:'cancel'});setSelectionMode(false);});
  window.TimDesktop=Object.freeze({
    ...
    resetWindow:()=>window.TimWindows.reset('projects'), arrange, selection:()=>[...selected], selectMode:value=>setSelectionMode(value)
  });
})();
