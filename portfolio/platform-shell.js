...
(() => {
  ...
  const top=document.createElement('nav');top.className='platform-topbar';top.setAttribute('aria-label','GNOME desktop panel');top.innerHTML='<button data-platform-applications>Applications</button><button data-platform-menu="places">Places</button><button data-platform-menu="system">System</button><button class="platform-clock" data-app-open="datetime" aria-label="Open Date/Time Properties"></button><button data-app-open="versions" title="Change operating system">Ubuntu ▾</button>';document.body.append(top);
  ...
  function sync(){start.setAttribute("aria-label",V.info().family==="mac"?"Macintosh menu":V.info().family==="linux"?"Applications menu":"Start");closeApps();closeStart();S.closeMenu();activeLabel();const brand=document.querySelector('.start-brand');if(V.info().family==='linux')brand.textContent=V.current()==='kde'?'KDE 3.5':'Ubuntu';window.dispatchEvent(new Event('resize'));}
