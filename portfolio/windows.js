...
(() => {
  ...
  function defaults(id){
    ...
    const sizes={browser:[1050,760],taskmanager:[540,430],naughty:[300,190],solitaire:[760,680],reversi:[530,680],minigolf:[830,720],documents:[560,380],'document-privacy':[730,710],'document-terms':[730,710],versions:[720,800],calculator:[350,460],datetime:[560,450],minesweeper:[380,465],notepad:[670,500],paint:[820,640],pinball:[960,840],run:[440,280],volume:[350,350],shutdown:[440,370],system:[490,440],computer:[570,430],accessories:[470,330],games:[420,330],settings:[540,360],recycle:[650,460],screensaver:[460,380]};
  ...
  function bound(r,w){const width=clamp(r.w,Math.min(w.tiled?120:compact()?250:w.main?540:340,innerWidth),innerWidth),height=clamp(r.h,Math.min(w.tiled?50:compact()?180:250,availableHeight()),availableHeight());return {x:clamp(r.x,0,innerWidth-width),y:clamp(r.y,workTop(),workBottom()-height),w:width,h:height};}
  function persist(){const geometry={};for(const [id,w] of registry)geometry[id]=compact()?(w.desktopRect||defaults(id)):(w.normal||w.rect);try{localStorage.setItem('timbuilds.windows.v3',JSON.stringify(geometry));}catch{}}
  function paint(w){
    ...
    const r=w.maximized?{x:0,y:workTop(),w:usableWidth,h:availableHeight()}:bound(w.rect,w);
  ...
  function close(id){const w=registry.get(id);if(!w)return;if(w.state==='closed')return;w.state='closed';w.el.hidden=true;w.task.hidden=true;w.onVisibility?.(false);w.onClose?.();if(active===id)fallback();else markActive(active);}
  function toggleTask(id){const w=registry.get(id);if(!w)return;if(w.state==='open'&&active===id)minimize(id);else show(id);}
  function maximize(id){const w=registry.get(id);if(!w)return;if(w.maximized){w.rect=bound(w.normal||defaults(id),w);w.normal=null;}else w.normal={...w.rect};w.maximized=!w.maximized;const button=w.el.querySelector('[data-win-control="maximize"],[data-action="maximize"]');window.TimChrome.set(button,w.maximized?'restore':'maximize');button.title=w.maximized?'Restore':'Maximise';button.setAttribute('aria-label',(w.maximized?'Restore':'Maximise')+' '+(w.main?'project window':w.label));paint(w);focusWindow(id);persist();}
  function register({id,el,label,icon,main=false,task=null,onClose=null,onVisibility=null}){
    if(!task){task=document.createElement('button');task.className='task-button';task.innerHTML=`<svg aria-hidden="true"><use href="portfolio/icons.svg?v=12#${safe(icon)}"/></svg><span>${safe(label)}</span>`;taskList.appendChild(task);}
    task.removeAttribute('data-action');task.dataset.windowTask=id;task.setAttribute('aria-controls',el.id);task.title=label;
    const w={id,el,label,main,task,onClose,onVisibility,state:'open',maximized:false,normal:null};w.rect=bound(!compact()&&valid(saved[id])?saved[id]:defaults(id),w);w.desktopRect=!compact()?{...w.rect}:null;w.mobileRect=compact()?{...w.rect}:null;
  ...
  function openPanel({id,title,label=title,html,icon='document',onClose=null,onVisibility=null}){
    ...
    if(!w){const el=document.createElement('section');el.id='window-'+id;el.className='window app-window';el.setAttribute('role','dialog');el.setAttribute('aria-modal','false');el.setAttribute('aria-labelledby',el.id+'-title');
      el.innerHTML=`<div class="titlebar"><div class="window-title"><svg aria-hidden="true"><use href="portfolio/icons.svg?v=12#${safe(icon)}"/></svg><span id="${el.id}-title">${safe(title)}</span></div><div class="window-controls"><button data-win-control="minimize" aria-label="Minimise ${safe(label)}" title="Minimise">_</button><button data-win-control="maximize" aria-label="Maximise ${safe(label)}" title="Maximise">□</button><button data-win-control="close" aria-label="Close ${safe(label)}" title="Close">×</button></div></div><div class="app-window-body"></div><div class="window-statusbar"><span>${safe(label)}</span><button class="resize-grip" data-resize="se" aria-label="Resize ${safe(label)} with arrow keys" title="Drag to resize, or use arrow keys">◢</button></div>`;
  ...
  document.addEventListener('pointerdown',event=>{
    ...
    if(event.button!==0||document.querySelector('dialog[open]')||w.maximized)return;
  ...
  function updateWindowDrag(event) {
    ...
    const minWidth = Math.min(compact()?250:windowState.main ? 540 : 340, innerWidth);
    const minHeight = Math.min(compact()?180:250, availableHeight());
    let { x, y, w, h } = start;
    if (!dir) { x += dx; y += dy; }
    else {
  ...
  document.addEventListener('keydown', event => {
    ...
    if (w.maximized || !['ArrowLeft','ArrowRight','ArrowUp','ArrowDown'].includes(event.key)) return;
