...
(() => {
  ...
  function contents(s){
    ...
    return `${display}<label class="response-label" for="access-response">Challenge response</label><input class="access-response" id="access-response" type="text" maxlength="80" autocomplete="off" autocapitalize="off" spellcheck="false" aria-describedby="access-hint" placeholder="Enter the response to this layer"><div id="access-rules" class="access-rules"></div>`;
  }
  function paintRules(){
  ...
  function showStage(){
    ...
    const auto=['memory','maze','order'].includes(stage.type);
    root.innerHTML=`<div class="access-terminal" data-depth="${depth}" data-challenge-type="${stage.type}"><div class="access-path"><span>C:\\Tim\\Locked</span><span class="access-state">RESTRICTED</span></div><div class="access-heading"><span class="access-lock" aria-hidden="true">▣</span><div><div class="eyebrow">ACCESS CONTROL / LAYER ${String(depth+1).padStart(4,'0')}</div><h2>${safe(stage.title)}</h2></div></div><p class="access-hint" id="access-hint"><strong>Recovery hint</strong>${safe(stage.hint)}</p><form id="access-form" autocomplete="off"><fieldset id="access-controls"><legend class="sr-only">Layer ${depth+1} challenge</legend>${contents(stage)}${auto?'':'<button class="bevel-button access-verify" type="submit">Verify response ↵</button>'}</fieldset></form><p id="access-feedback" class="access-feedback" role="status" aria-live="polite">${depth?'Previous layer accepted. Additional verification required.':'Folder access requires verification.'}</p><div id="access-accepted" class="access-accepted" hidden><strong>Layer accepted.</strong><p>A further verification layer was discovered.</p><button class="bevel-button" data-access-action="next">Continue to next layer →</button></div><div class="access-footer"><span id="access-attempts">Attempts at this layer: 0</span><button class="text-button" data-access-action="disconnect">Disconnect</button></div></div>`;
    paintRules();root.querySelector('#access-response')?.focus({preventScroll:true});root.scrollTop=0;remember();
  }
  function reject(message='Response rejected. Consult the recovery hint and try again.'){
    attempts++;root.querySelector('#access-attempts').textContent=`Attempts at this layer: ${attempts}`;feedback(message);
  }
  function accept(){
    if(solved)return;solved=true;stopPlayback();root.querySelector('#access-controls').disabled=true;
    root.querySelector('#access-accepted').hidden=false;root.querySelector('.access-state').textContent='LAYER VERIFIED';feedback('Verification accepted. Folder remains restricted.');
    root.querySelector('[data-access-action="next"]').focus({preventScroll:true});
  }
  function verify(){
    if(solved)return;const input=root.querySelector('#access-response');
  ...
  function replay(){
    if(stage.type!=='memory'||solved)return;stopPlayback();entry=[];root.querySelector('#sequence-entry').textContent='Sequence entered: —';
  ...
  function click(event) {
    ...
    if (action==='disconnect') { window.TimWindows.close('locked'); return; }
    if (action==='next' && solved) { depth++; showStage(); return; }
    if (solved) return;
    ...
    if (button.dataset.tile!==undefined && stage.type==='tiles') {
      ...
      button.setAttribute('aria-pressed',String(entry.includes(n)));
      return;
    }
    if (button.dataset.pad && stage.type==='memory' && !playing) {
      ...
      } else { entry.push(n); feedback('Boot signal accepted.'); }
    ...
    if (button.dataset.sector && stage.type==='order') {
      ...
      } else { entry.push(n); button.disabled=true; button.classList.add('is-complete'); }
  ...
  function mount(body) {
    ...
    root.addEventListener('input',paintRules,options);
