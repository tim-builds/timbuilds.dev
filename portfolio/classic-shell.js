...
(() => {
  ...
  A.register('run','Run','computer',(body)=>{
    const commands={winver:'versions',themes:'versions',notepad:'notepad',calc:'calculator',calculator:'calculator',mspaint:'paint',paint:'paint',winmine:'minesweeper',minesweeper:'minesweeper',pinball:'pinball',control:'settings',timedate:'datetime',explorer:'computer',projects:'projects',help:'help'};
  ...
  function contextItems(target){
    ...
    if(target.closest('.taskbar'))return [{label:'Cascade windows',run:()=>W.arrange('cascade')},{label:'Tile windows horizontally',run:()=>W.arrange('horizontal')},{label:'Tile windows vertically',run:()=>W.arrange('vertical')},null,{label:'Show desktop',run:()=>W.showDesktop()},{label:'Restore windows',run:()=>W.restoreDesktop()},null,{label:'Windows version…',run:()=>launch('versions')}];
    if(target.closest('.desktop-dock')||target===document.body)return [{label:'Arrange icons on left',run:()=>D.arrange()},{label:'Refresh',run:()=>{window.dispatchEvent(new Event('resize'));beep();}},null,{label:'New Text Document',run:()=>launch('notepad')},{label:'Run…',run:()=>launch('run')},null,{label:'Windows version…',run:()=>launch('versions')},{label:'Properties',run:()=>launch('display')}];
    return null;
  }
