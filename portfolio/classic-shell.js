...
(() => {
  ...
  const categories={accessories:['notepad','calculator','paint'],games:['minesweeper','pinball'],settings:['datetime','volume','system']};
  ...
  A.register('system','System Properties','computer',(body)=>{
    body.innerHTML='<div class="accessory-pad"><div class="classic-tabs"><button class="selected">General</button></div><h2>timBuilds Desktop</h2><p>A Windows 95-style environment, running entirely in your web browser.</p><dl class="system-specs"><dt>System</dt><dd>Browser desktop · release 4</dd><dt>Display</dt><dd>'+innerWidth+' × '+innerHeight+' CSS pixels</dd><dt>Programs</dt><dd>'+A.list().length+' web accessories and folders</dd><dt>Personal files</dt><dd>Notes stored locally in this browser</dd><dt>Owner folder</dt><dd>Encrypted; independent of the games</dd></dl><p>No Windows installation, account or device access is required.</p><a href="portfolio/THIRD-PARTY-NOTICES.md" target="_blank" rel="noopener">Credits and asset sources ↗</a></div>';
  });
  A.register('run','Run','computer',(body)=>{
    const commands={notepad:'notepad',calc:'calculator',calculator:'calculator',mspaint:'paint',paint:'paint',winmine:'minesweeper',minesweeper:'minesweeper',pinball:'pinball',control:'settings',timedate:'datetime',explorer:'computer',projects:'projects',help:'help'};
  ...
  function contextItems(target){
    ...
    if(task||title){const id=task?task.dataset.windowTask:title.parentElement.dataset.windowId,w=W.list().find(x=>x.id===id);return [{label:'Restore',run:()=>{W.show(id);if(w.maximized)W.maximize(id);},disabled:w.state==='open'&&!w.maximized},{label:'Move',run:()=>{W.show(id);document.querySelector(`[data-window-id="${id}"] > .titlebar`).focus();},disabled:w.maximized},{label:'Size',run:()=>{W.show(id);document.querySelector(`[data-window-id="${id}"] .resize-grip`).focus();},disabled:w.maximized},{label:'Minimise',run:()=>W.minimize(id),disabled:w.state!=='open'},{label:'Maximise',run:()=>{W.show(id);if(!w.maximized)W.maximize(id);},disabled:w.maximized},null,{label:'Close',run:()=>W.close(id)}];}
    if(target.closest('.clock-tray'))return [{label:'Adjust Date/Time…',run:()=>launch('datetime')},{label:'Volume Control',run:()=>launch('volume')}];
    if(target.closest('.taskbar'))return [{label:'Cascade windows',run:()=>W.arrange('cascade')},{label:'Tile windows horizontally',run:()=>W.arrange('horizontal')},{label:'Tile windows vertically',run:()=>W.arrange('vertical')},null,{label:'Show desktop',run:()=>W.showDesktop()},{label:'Restore windows',run:()=>W.restoreDesktop()}];
    if(target.closest('.desktop-dock')||target===document.body)return [{label:'Arrange icons on left',run:()=>D.arrange()},{label:'Refresh',run:()=>{window.dispatchEvent(new Event('resize'));beep();}},null,{label:'New Text Document',run:()=>launch('notepad')},{label:'Run…',run:()=>launch('run')},null,{label:'Properties',run:()=>launch('display')}];
    return null;
  }
  ...
  nav.innerHTML='<div class="start-group"><button class="start-parent" aria-expanded="false">'+icon('folder')+'<span>Programs</span><b>▸</b></button><div class="start-submenu window">'+menuButton('Accessories','accessories','folder')+menuButton('Notepad','notepad','notepad')+menuButton('Calculator','calculator','calculator')+menuButton('Paint','paint','paint')+'<hr>'+menuButton('Minesweeper','minesweeper','mine')+menuButton('Pinball','pinball','pinball')+'</div></div><button data-action="projects">'+icon('folder')+'<span>My Projects</span></button>'+menuButton('My Computer','computer','computer')+'<div class="start-group"><button class="start-parent" aria-expanded="false">'+icon('settings')+'<span>Settings</span><b>▸</b></button><div class="start-submenu window"><button data-dialog="display">'+icon('palette')+'Display</button>'+menuButton('Date/Time','datetime','clock')+menuButton('Volume','volume','speaker')+menuButton('Control Panel','settings','settings')+menuButton('Screen Saver','screensaver','computer')+'</div></div><button data-dialog="help">'+icon('document')+'Help</button>'+menuButton('Run…','run','computer')+'<hr>'+menuButton('Shut Down…','shutdown','power');
  ...
  window.TimShell=Object.freeze({launch,closeMenu,beep});
})();
