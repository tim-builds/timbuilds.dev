...
`REPOSITORY-CONSOLIDATION.md` records the repository reconciliation and remaining acceptance items. Issue #20 remains the migration evidence index. The old `tim-builds/site` repository is retained as historical/rollback material, not a second place to make current changes. Old interim authoring instructions, including references from the native app's historical migration runbook, are superseded **for website publishing only** by this README. They do not authorize native-app or account changes.
