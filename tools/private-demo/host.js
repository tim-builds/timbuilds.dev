...
(async()=>{
 ...
 A.register('private-candystand','Candystand - Private demos','folder',(body)=>{
  body.innerHTML='<div style="padding:16px"><p>Owner-only demos. Choose a game; rotate your phone sideways for more space.</p><div data-private-games style="display:grid;gap:12px"></div><p>Touchpad: slide to aim, tap anywhere to click, or keep your finger down briefly, then drag while holding to aim/shoot. Lift your finger to release the mouse. The black margins work too. Direct touch is also available.</p><button class="bevel-button" data-lock style="min-height:44px">Lock private demo</button></div>';
 ...
 for(const entry of entries)A.register('lab-'+entry.id,entry.title+' - Private','globe',(body,win)=>{
  ...
  function canInput(){return !closed&&!userPaused&&visible&&!document.hidden;}
  function heldFeedback(){const down=held||gestureHeld;win.dataset.classicMouseHeld=String(down);cursor.querySelector('path').setAttribute('fill',down?'#ffe081':'white');const button=body.querySelector('[data-classic-hold]');button.textContent=down?'Release':'Hold';button.setAttribute('aria-pressed',String(down));}
  function release(cancelled=false){if(clickTimer){clearTimeout(clickTimer);clickTimer=0;input(cancelled?'cancel':'up');}if(held||gestureHeld)input(cancelled?'cancel':'up');held=false;gestureHeld=false;heldFeedback();}
  function press(){if(!canInput()||held||gestureHeld||clickTimer)return false;gestureHeld=true;input('move');input('down');heldFeedback();return true;}
  function releaseGesture(cancelled){if(!gestureHeld)return;gestureHeld=false;input(cancelled?'cancel':'up');heldFeedback();}
  function mode(){release(true);pad.hidden=!trackpad;cursor.hidden=!trackpad;body.querySelector('[data-classic-touch]').textContent=trackpad?'Direct touch':'Touchpad';for(const key of ['tap','hold'])body.querySelector('[data-classic-'+key+']').hidden=!trackpad;}
  function tap(){if(closed||userPaused||!visible||document.hidden||clickTimer)return;if(held||gestureHeld){release();return;}input('move');input('down');clickTimer=setTimeout(()=>{clickTimer=0;if(!closed)input('up');},80);}
  const touch=window.TimDemoTouchpad.mount({element:pad,move:(dx,dy)=>{x=Math.max(1,Math.min(639,x+dx/Math.max(.1,scale)));y=Math.max(1,Math.min(419,y+dy/Math.max(.1,scale)));draw();input('move');},tap,press,release:releaseGesture,cancel:()=>release(true),enabled:canInput});
  ...
  body.querySelector('[data-classic-hold]').onclick=()=>{if(!canInput())return;if(held||gestureHeld){touch.cancel();release();}else{if(clickTimer)release(true);held=true;input('move');input('down');heldFeedback();}};
