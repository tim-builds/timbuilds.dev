...
try{
 ...
 await send('Page.enable');await send('Runtime.enable');await send('Network.enable');await viewport(1440,1100);await navigate(origin);
 ...
 if(fs.existsSync(ownerFile)){
  ...
  for(const version of ['95','98','2000','xp','system7','mac9','ubuntu','kde']){await evaluate('window.TimVersion.set('+JSON.stringify(version)+')');await click('[data-action="locked"]');assert.equal(await evaluate('document.querySelectorAll(".project-card:not([hidden])").length'),7);assert.equal(await evaluate('document.querySelector("#detail-dialog").open'),false);}pass('Owner access and all seven private entries survive switching between every desktop platform');
  ...
  await evaluate(`new Promise((resolve,reject)=>{const r=indexedDB.deleteDatabase('timbuilds-owner-v1');r.onsuccess=resolve;r.onerror=()=>reject(r.error);})`);await navigate(origin);await click('[data-action="locked"]');await until(()=>evaluate('!!document.querySelector("#window-locked:not([hidden]) .access-terminal")'),'guest terminal after owner-key lookup');assert.ok(await evaluate('!!document.querySelector(".access-terminal")'));await click('#window-locked [data-win-control="close"]');pass('Clearing the browser key removes owner access');
 }
 const blocked=await send('Page.addScriptToEvaluateOnNewDocument',{source:`Object.defineProperty(window,'localStorage',{get(){throw new DOMException('denied','SecurityError')}});Object.defineProperty(window,'indexedDB',{get(){throw new DOMException('denied','SecurityError')}});`});await navigate(origin);await click('[data-action="locked"]');await until(()=>evaluate('!!document.querySelector("#window-locked:not([hidden]) .access-terminal")'),'guest terminal after owner-key lookup');assert.ok(await evaluate('!!document.querySelector(".access-terminal")'));await click('#window-locked [data-win-control="close"]');await send('Page.removeScriptToEvaluateOnNewDocument',{identifier:blocked.identifier});pass('Blocked browser storage does not break public browsing or the puzzle');
