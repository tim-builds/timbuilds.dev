...
import {checkFolderExtension} from './check-folder-extension.mjs';
import {checkClockEra} from './check-clock-era.mjs';
import {checkDesktopDefaults} from './check-desktop-defaults.mjs';
import {checkPinballSizing} from './check-pinball-sizing.mjs';
...
try{
 ...
 for(const [category,count] of ['Apps','Games','Websites','Tools','Experiments','All projects'].map(c=>[c,c==='All projects'?catalogue.length:catalogue.filter(p=>p.category===c).length])){await evaluate(`window.TimCatalogue.browse("${category}")`);assert.equal(await evaluate('document.querySelectorAll(".project-card:not([hidden])").length'),count);}pass('Every category filter');
 ...
 await evaluate('window.TimCatalogue.setView("list")');assert.ok(await evaluate('document.querySelector("#project-grid").classList.contains("is-list")'));await evaluate('window.TimCatalogue.setView("grid")');pass('Grid and list views');
 ...
 await checkClockEra({evaluate,send,click,box,mouse,drag,viewport,navigate,origin,until,sleep,screenshot,output,pass,requests});
 await checkDesktopDefaults({evaluate,click,box,viewport,navigate,origin,screenshot,output,pass});
 await checkPinballSizing({evaluate,send,click,box,mouse,drag,viewport,navigate,origin,until,sleep,screenshot,output,pass,requests});
 ...
 await evaluate('localStorage.setItem("owner","true");localStorage.setItem("timbuilds.owner","true")');await navigate(origin);await evaluate('window.TimCatalogue.browse("Locked")');await until(()=>evaluate('!!document.querySelector("#bsod-dialog[open] .access-terminal")'),'guest terminal after owner-key lookup');assert.ok(await evaluate('!!document.querySelector(".access-terminal")'));await click('#bsod-dialog [data-bsod-action="desktop"]');pass('Spoofing an owner preference does not bypass encryption');
 ...
 if(fs.existsSync(ownerFile)){
  ...
  await navigate(origin);await evaluate('window.TimCatalogue.browse("Locked")');assert.equal(await evaluate('document.querySelectorAll(".project-card:not([hidden])").length'),8);assert.equal(await evaluate('document.querySelector("#detail-dialog").open'),false);pass('Saved owner key automatically opens Locked after a real reload');
  for(const version of ['95','98','2000','xp','system7','mac9','ubuntu','kde']){await evaluate('window.TimVersion.set('+JSON.stringify(version)+')');await evaluate('window.TimCatalogue.browse("Locked")');assert.equal(await evaluate('document.querySelectorAll(".project-card:not([hidden])").length'),8);assert.equal(await evaluate('document.querySelector("#detail-dialog").open'),false);}pass('Owner access and all eight private entries survive switching between every desktop platform');
  assert.equal(await evaluate('document.querySelector(".locked-project[data-id=cham-care]").hidden'),false);await evaluate('window.TimPower.start("restart")');await click('.power-skip');await click('.power-skip');await evaluate('window.TimCatalogue.browse("Locked")');await until(()=>evaluate('document.querySelectorAll(".locked-project:not([hidden])").length===8'),'owner folder after restart');pass('ChamCare is owner-only and restarting preserves all eight encrypted catalogue entries');
  await evaluate('window.TimVersion.set("95")');
  await evaluate('window.TimCatalogue.browse("All projects")');assert.equal(await evaluate('document.querySelectorAll(".locked-project:not([hidden])").length'),0);pass('Private entries remain excluded from All projects even for the owner');
  ...
  await evaluate(`new Promise((resolve,reject)=>{const r=indexedDB.deleteDatabase('timbuilds-owner-v1');r.onsuccess=resolve;r.onerror=()=>reject(r.error);})`);await navigate(origin);await evaluate('window.TimCatalogue.browse("Locked")');await until(()=>evaluate('!!document.querySelector("#bsod-dialog[open] .access-terminal")'),'guest terminal after owner-key lookup');assert.ok(await evaluate('!!document.querySelector(".access-terminal")'));await click('#bsod-dialog [data-bsod-action="desktop"]');pass('Clearing the browser key removes owner access');
 }
 const blocked=await send('Page.addScriptToEvaluateOnNewDocument',{source:`Object.defineProperty(window,'localStorage',{get(){throw new DOMException('denied','SecurityError')}});Object.defineProperty(window,'indexedDB',{get(){throw new DOMException('denied','SecurityError')}});`});await navigate(origin);await evaluate('window.TimCatalogue.browse("Locked")');await until(()=>evaluate('!!document.querySelector("#bsod-dialog[open] .access-terminal")'),'guest terminal after owner-key lookup');assert.ok(await evaluate('!!document.querySelector(".access-terminal")'));await click('#bsod-dialog [data-bsod-action="desktop"]');await send('Page.removeScriptToEvaluateOnNewDocument',{identifier:blocked.identifier});pass('Blocked browser storage does not break public browsing or the puzzle');
