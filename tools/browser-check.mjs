...
try{
 ...
 await click('[data-action="locked"]');assert.ok(await evaluate('!!document.querySelector("#guess-choices")'));await screenshot(path.join(output,'locked-game.png'));for(let i=0;i<40;i++)await click('[data-guess-choice="0"]');assert.equal(await evaluate('document.querySelectorAll(".locked-project").length'),0);assert.equal(await evaluate('document.querySelectorAll("#guess-hints li").length'),6);assert.equal(await evaluate('document.querySelector("#guess-count").textContent'),'40');await click('.dialog-close');pass('Guest puzzle escalates, keeps a bounded hint history, never grants access and closes');
 await evaluate('localStorage.setItem("owner","true");localStorage.setItem("timbuilds.owner","true")');await navigate(origin);await click('[data-action="locked"]');assert.ok(await evaluate('!!document.querySelector("#guess-choices")'));await click('.dialog-close');pass('Spoofing an owner preference does not bypass encryption');
 ...
 if(fs.existsSync(ownerFile)){
  ...
  await evaluate(`new Promise((resolve,reject)=>{const r=indexedDB.deleteDatabase('timbuilds-owner-v1');r.onsuccess=resolve;r.onerror=()=>reject(r.error);})`);await navigate(origin);await click('[data-action="locked"]');assert.ok(await evaluate('!!document.querySelector("#guess-choices")'));await click('.dialog-close');pass('Clearing the browser key removes owner access');
 }
 const blocked=await send('Page.addScriptToEvaluateOnNewDocument',{source:`Object.defineProperty(window,'localStorage',{get(){throw new DOMException('denied','SecurityError')}});Object.defineProperty(window,'indexedDB',{get(){throw new DOMException('denied','SecurityError')}});`});await navigate(origin);await click('[data-action="locked"]');assert.ok(await evaluate('!!document.querySelector("#guess-choices")'));await click('.dialog-close');await send('Page.removeScriptToEvaluateOnNewDocument',{identifier:blocked.identifier});pass('Blocked browser storage does not break public browsing or the puzzle');
