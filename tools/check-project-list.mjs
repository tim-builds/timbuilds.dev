...
export async function checkProjectList({evaluate,send,click,box,mouse,viewport,navigate,origin,until,sleep,screenshot,output,pass,getTargets}){
 ...
 const themes=await evaluate('window.TimVersion.themes.map(t=>t.id)');assert.equal(themes.length,11);
 for(const os of themes){
  await evaluate(`window.TimVersion.set('${os}');window.TimWindows.show('projects');window.TimCatalogue.browse('All projects');window.TimCatalogue.setView('list')`);
  for(const [width,height]of [[320,740],[390,844],[844,390],[1440,1000]]){
   await viewport(width,height);await evaluate('document.querySelector(".workspace").scrollTop=0');await sleep(100);
   assert.equal(await evaluate('document.documentElement.scrollWidth>innerWidth'),false,os+' no page overflow '+width);
   const layout=await evaluate(`(()=>{const rows=[...document.querySelectorAll('.project-card:not([hidden])')],workspace=document.querySelector('.workspace');return {overflow:workspace.scrollWidth>workspace.clientWidth+1,rows:rows.map(el=>{const r=el.getBoundingClientRect(),s=getComputedStyle(el),t=el.querySelector('h3').getBoundingClientRect(),p=el.querySelector('.card-content>p').getBoundingClientRect(),img=el.querySelector('img'),im=img.getBoundingClientRect();return {x:r.x,y:r.y,w:r.width,h:r.height,bottom:r.bottom,border:s.borderTopWidth,titleFits:t.right<=r.right&&t.bottom<=r.bottom,bodyFits:p.right<=r.right&&p.bottom<=r.bottom,fit:getComputedStyle(img).objectFit,ratio:im.width/im.height,font:parseFloat(getComputedStyle(el.querySelector('.card-content>p')).fontSize)}})};})()`);
   assert.equal(layout.overflow,false,os+' no inner overflow '+width);assert.equal(layout.rows.length,13);
   for(let i=0;i<layout.rows.length;i++){const row=layout.rows[i];assert.equal(row.border,'1px');assert.equal(row.titleFits,true);assert.equal(row.bodyFits,true);assert.equal(row.fit,'contain');assert.ok(Math.abs(row.ratio-1.5)<.015);assert.ok(row.font>=13);if(i)assert.ok(row.y-layout.rows[i-1].bottom>=11,'visible separation '+os+' '+width);}
   if(['xp','11','vista'].includes(os)&&[390,1440].includes(width))await screenshot(path.join(output,`clean-list-${os}-${width}.png`));
  }
 }
 await viewport(1440,1000);await evaluate('window.TimVersion.set("xp");document.querySelectorAll(".project-card img").forEach(i=>i.loading="eager")');await until(()=>evaluate('[...document.querySelectorAll(".project-card img")].every(i=>i.complete&&i.naturalWidth===600&&i.naturalHeight===400)'),'all13 actual thumbnails loaded');
 await command('Games');assert.equal(await evaluate('document.querySelectorAll(".project-card:not([hidden])").length'),2);assert.equal(await evaluate('document.querySelector("#collection-title").textContent'),'Games');await command('All projects');
 await command('Cards');assert.equal(await evaluate('window.TimCatalogue.view()'),'grid');await command('List');assert.equal(await evaluate('window.TimCatalogue.view()'),'list');
