...
export async function checkClockEra({evaluate,click,box,mouse,viewport,navigate,origin,sleep,screenshot,output,pass}){
 ...
 for(const os of ['95','98','2000','xp','system7','mac9','ubuntu','kde'])for(const width of [320,390,1440]){
  ...
  if(['95','98','2000','xp'].includes(os)){const section=await evaluate("(()=>{const e=document.querySelector('.taskbar>.clock-era'),s=getComputedStyle(e);return {border:parseFloat(s.borderLeftWidth),background:s.backgroundImage,color:s.backgroundColor};})()");assert.ok(section.border>=1,os+' shared tray boundary');if(os==='xp')assert.notEqual(section.background,'none','XP shared tray shading');}
 ...
 pass('Integrated time/year: consistent type and baseline, only year underlined, shared Windows tray section, no dot/chevron/individual bevels, h:mm AM/PM, real clock/era toggles and live year switching across 8 skins at 320/390/1440px');
}
