...
export async function checkProjectSites({evaluate,send,click,box,mouse,drag,viewport,navigate,origin,until,sleep,screenshot,output,pass,requests}){
 ...
 await mouse(bin.x+bin.w/2,bin.y+bin.h/2,'mouseReleased');await until(()=>evaluate('document.querySelector("#desktop-warning").open'),'protected folder warning');assert.equal(await evaluate('document.querySelector("#recycle-warning-heading").textContent'),'Hey!');assert.equal(await evaluate('document.querySelectorAll(".desktop-drag-copy").length'),0);assert.deepEqual(await box('[data-shortcut=projects]'),before);await screenshot(path.join(output,'polished-recycle-warning.png'));await click('#desktop-warning .warning-ok');
