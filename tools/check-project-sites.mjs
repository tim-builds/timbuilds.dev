...
export async function checkProjectSites({evaluate,send,click,box,mouse,drag,viewport,navigate,origin,until,sleep,screenshot,output,pass,requests}){
 ...
 const frameClick=async selector=>{const p=await evaluate(`(()=>{const f=document.querySelector('.browser-frame'),a=f.contentDocument.querySelector(${JSON.stringify(selector)});a.scrollIntoView({block:'center'});const fr=f.getBoundingClientRect(),r=a.getBoundingClientRect();return {x:fr.x+r.x+r.width/2,y:fr.y+r.y+r.height/2};})()`);await mouse(p.x,p.y,'mousePressed');await mouse(p.x,p.y,'mouseReleased');};
 ...
 await mouse(bin.x+bin.w/2,bin.y+bin.h/2,'mouseReleased');await until(()=>evaluate('document.querySelector("#desktop-warning").open'),'protected folder warning');assert.equal(await evaluate('document.querySelector("#recycle-warning-heading").textContent'),'Hey! Nice try.');assert.equal(await evaluate('document.querySelectorAll(".desktop-drag-copy").length'),0);assert.deepEqual(await box('[data-shortcut=projects]'),before);await screenshot(path.join(output,'polished-recycle-warning.png'));await click('#desktop-warning .warning-ok');
 pass('Dragging uses a translucent copy and bin highlight; a refused drop returns cleanly and opens an OS-style Hey! warning');
 ...
 await send('Emulation.setEmulatedMedia',{features:[{name:'prefers-reduced-motion',value:'reduce'}]});await evaluate('window.TimVersion.set("2000");window.TimDesktop.arrange()');await drag('[data-shortcut=projects]',0,96);assert.equal(await evaluate('document.querySelector("#desktop-warning").open'),true);assert.equal(await evaluate('document.querySelectorAll(".desktop-drag-copy").length'),0);await click('#desktop-warning .warning-ok');await send('Emulation.setEmulatedMedia',{features:[]});
