...
export async function checkShell({evaluate,send,click,box,mouse,drag,viewport,navigate,origin,until,sleep,screenshot,output,pass,requests}){
  ...
  assert.equal((await box('.desktop-shortcut[data-action="projects"]')).x,220);assert.equal((await box('.desktop-shortcut[data-dialog="about"]')).x,220);assert.equal((await box('.desktop-shortcut[data-dialog="about"]')).y-(await box('.desktop-shortcut[data-action="projects"]')).y,96);
  ...
  await evaluate('window.TimDesktop.arrange()');await drag('.desktop-shortcut[data-dialog="display"]',104,-384);assert.equal((await box('.desktop-shortcut[data-dialog="display"]')).y,12);pass('Blank click clears selection and the entire top desktop row accepts icons');
  ...
  for(const id of ['about','contact','github','display'])await click(`.desktop-shortcut[data-dialog="${id}"]`);
  assert.equal(await evaluate('document.querySelectorAll(".app-window:not([hidden])").length'),4);assert.equal(await evaluate('document.querySelectorAll(".task-button:not([hidden])").length'),5);assert.equal(await evaluate('document.querySelector("#detail-dialog").open'),false);pass('All four auxiliary icons open independent, non-modal windows with taskbar buttons');
  ...
  assert.equal(backgrounds.size,12);await click('[data-wallpaper="synthwave"]');await screenshot(path.join(output,'retro-windows.png'));pass('Twelve distinct wallpapers have working live previews and selection states');
  ...
  for(let n=0;n<24;n++){
    ...
    if(stage.type==='tiles'){
      ...
      await click('[data-access-action="replay"]');await sleep(80);await click('[data-window-task="locked"]');await sleep(80);await click('[data-window-task="locked"]');assert.equal(await evaluate('document.querySelectorAll(".sequence-pad:disabled").length'),0);
  ...
  await navigate(origin+'/?shell-resume=3');await click('[data-action="locked"]');await until(()=>evaluate('!!document.querySelector(".access-terminal")'),'resumed terminal');assert.equal(await evaluate('document.querySelector(".access-terminal").dataset.depth'),'24');await click('#window-locked [data-win-control="close"]');pass('Closing and reloading preserve the current layer without retaining entered responses');
  for(const width of [320,390,768]){
    await viewport(width,844);await click('.desktop-shortcut[data-dialog="display"]');const display=await box('#window-display');assert.ok(display.x>=0&&display.x+display.w<=width);assert.equal(await evaluate('document.querySelector("#window-display .app-window-body").scrollWidth>document.querySelector("#window-display .app-window-body").clientWidth'),false);await click('#window-display [data-win-control="close"]');
    await click('[data-action="locked"]');const access=await box('#window-locked');assert.ok(access.x>=0&&access.x+access.w<=width);assert.equal(await evaluate('document.querySelector("#window-locked .app-window-body").scrollWidth>document.querySelector("#window-locked .app-window-body").clientWidth'),false);if(width===320)await screenshot(path.join(output,'access-mobile.png'));await click('#window-locked [data-win-control="close"]');
