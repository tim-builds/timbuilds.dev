...
export async function checkShell({evaluate,send,click,box,mouse,drag,viewport,navigate,origin,until,sleep,screenshot,output,pass,requests}){
  ...
  await evaluate('sessionStorage.setItem("timbuilds.access-progress.v3",JSON.stringify({depth:0,seed:123456}))');await navigate(origin+'/?shell-test=3');await evaluate('window.TimCatalogue.browse("Locked")');await until(()=>evaluate('!!document.querySelector(".access-terminal")'),'access terminal');await sleep(150);
  ...
  for(const width of [320,390,768]){
    ...
    await evaluate('window.TimCatalogue.browse("Locked")');const access=await box('#bsod-dialog');assert.ok(access.x>=0&&access.x+access.w<=width);assert.equal(await evaluate('document.querySelector("#bsod-dialog .bsod-scroll").scrollWidth>document.querySelector("#bsod-dialog .bsod-scroll").clientWidth'),false);if(width===320)await screenshot(path.join(output,'access-mobile.png'));await click('#bsod-dialog [data-bsod-action="desktop"]');
