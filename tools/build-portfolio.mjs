...
export const isWebsiteLink = url => /^(?:https:\/\/|\/(?!\/))/i.test(url);
export function build() {
  const projects=JSON.parse(fs.readFileSync(path.join(root,'portfolio/projects.json'),'utf8'));
  const cards=projects.map(p=>`<article class="project-card" data-id="${p.id}" data-category="${p.category}"><button type="button" class="card-preview" data-project="${p.id}" aria-label="Preview ${escapeHTML(p.title)}"><img src="${escapeHTML(p.thumbnail||p.image)}" width="600" height="400" alt="${escapeHTML(p.thumbnailAlt||p.imageAlt||p.title)}" loading="lazy" decoding="async"></button><div class="card-content"><div class="card-category">${escapeHTML(p.category)}</div><h3><button data-project="${p.id}" aria-haspopup="dialog">${escapeHTML(p.title)}</button></h3><p>${escapeHTML(p.summary)}</p><div class="card-footer"><span class="project-status${['Playable','Interactive demo','You are here'].includes(p.status)?' status-public':''}">${escapeHTML(p.status)}</span>${p.url?`<a href="${escapeHTML(p.url)}"${isWebsiteLink(p.url)?' data-project-website target="_blank" rel="noopener noreferrer"':''}>${escapeHTML(p.cta)} <span aria-hidden="true">↗</span>${isWebsiteLink(p.url)?'<span class="sr-only"> (opens in a new browser tab)</span>':''}</a>`:`<button data-project="${p.id}" aria-label="Details about ${escapeHTML(p.title)}">Details <span aria-hidden="true">→</span></button>`}</div></div></article>`).join('\n');
